...
namespace MatchThree.Core
{
    public class Board : MonoBehaviour
    {
        ...
        private void PlayIntroEffects()
        {
            ...
            if (gameSettings.EnableVFX)
            {
                for (int i = 0; i < boardSize.x; i++)
                {
                    for (int j = 0; j < boardSize.y; j++)
                    {
                        boardTileMap[i, j].PopVisual(scale: 0.5f, duration: .25f, vibrato: 2, elasticity: .05f);
                    }
                }
            }
        }
        ...
        bool FindMatchesAtIndex(Vector2Int posIdx)
        {
            ...
            if (focusedGamePiece.IsMarked) return false;
        ...
        int MarkGamePieces(List<MatchResult> results)
        {
            ...
            {
                if (!result.GamePiece.IsMarked)
                {
                    result.GamePiece.Mark();
                    score++;
                }
            }
        ...
        private void PlayNoMoreMatchesEffects()
        {
            ...
            if (gameSettings.EnableVFX)
            {
                for (int i = 0; i < boardSize.x; i++)
                {
                    for (int j = 0; j < boardSize.y; j++)
                    {
                        boardTileMap[i, j].PopVisual(scale: 1f, duration: .45f, vibrato: 5, elasticity: 1f);
